...
try:
  ...
  for dbName in cursor.fetchall():
    if dbName[0] == "streamteste":
      print("O banco de dados já está criado.")
      break
    else:
      cursor.execute("create database if not exists streamteste;")
      connection.database = 'streamteste'
      print("O banco não existe, mas já estamos criando para você!")
      break
